...
class ResNetFeature(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.config['seq_len']):
            x1 = x[:, i].view(x.size(0), 1, -1)
            f = self.initial_layer(x1)  # [128, 1, 3000] [128, 16, 750]
            f = self.layer1(f)
            f = self.layer2(f)
            f = self.maxpool(f)
            f = self.layer3(f)
        ...
        out1 = torch.cat(f_seq, dim=1)
        return out1


class ResNetFeature_1500(nn.Module):

    def __init__(self, config):

        super(ResNetFeature_1500, self).__init__()
    ...
    def forward(self, x):
        ...
        # print(x.shape)
        for i in range(self.config['seq_len']):
            f = self.initial_layer(x[:, i].view(x.size(0), 1, -1))  # [128, 1, 3000] [128, 16, 750]
            f = self.layer1(f)
            f = self.layer2(f)
            f = self.maxpool(f)
            f = self.layer3(f)
